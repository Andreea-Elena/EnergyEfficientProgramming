...
#define l_checkmode(mode)                                                                                          \
  (*mode != '\0' && strchr("rwa", *(mode++)) != NULL && (*mode != '+' || (++mode, 1)) && /* skip if char is '+' */ \
...
static int g_write(lua_State *L, FILE *f, int arg) {
  ...
  for (; nargs--; arg++) {
    if (lua_type(L, arg) == LUA_TNUMBER) {
      ...
      int len = lua_isinteger(L, arg) ? fprintf(f, LUA_INTEGER_FMT, lua_tointeger(L, arg)) : fprintf(f, LUA_NUMBER_FMT, lua_tonumber(L, arg));
      status = status && (len > 0);
    } else {
      size_t l;
...
*/
static const luaL_Reg iolib[] = {{"close", io_close},     {"flush", io_flush},   {"input", io_input}, {"lines", io_lines},
                                 {"open", io_open},       {"output", io_output}, {"popen", io_popen}, {"read", io_read},
                                 {"tmpfile", io_tmpfile}, {"type", io_type},     {"write", io_write}, {NULL, NULL}};
